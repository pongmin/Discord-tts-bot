...
Run ``python -m tests.test_ban_algorithm`` (optionally ``--db /path/to/scouting.db``)
from the repo root. The collected database is opened read-only and copied into
memory. All fixture writes happen there; this script makes no API calls or
reads of .env. Inspect the printed Pongmin diagnostics by eye; this is not an
evaluation harness.
"""
